...
def update(replay_buffer: ReplayMemory, batch_size: int, net: torch.nn.Module, optimizer: torch.optim, loss_fn=compute_loss_dqn) -> None:
    batch = get_batch(replay_buffer, batch_size)
    if batch is None:
        return
    optimizer.zero_grad()
    loss = loss_fn(batch, net)
    loss.backward()
...
def deep_q_network(q_net, env, alpha=1e-4, weight_decay=1e-5, batch_size=10, exploration_rate=1, exploration_decay=(1-1e-3), exploration_min=0, num_episodes=1000, max_episode_length=np.iinfo(np.int32).max, train=True, print_res=True, print_freq=100) -> tuple[np.ndarray, np.ndarray]: 
    """
    Training for DQN
    ...
    for n in range(num_episodes):
        ...
        for i in range(max_episode_length): 
            ...
            if train and len(replay_buffer) >= batch_size:
                update(replay_buffer, batch_size, q_net, optimizer)
